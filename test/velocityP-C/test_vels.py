...
def compare_vels(tol, md_fname="md_vels.dat",
                   openfoam_dir="test_vels_case", steps=2, dt=2, mode="test"):
    for step in xrange(1, steps+1):
        openfoam_dir_step = os.path.join(openfoam_dir, str(step*dt))
        vel = ParsedParameterFile(os.path.join(openfoam_dir_step, "U"))["boundaryField"]["CPLReceiveMD"]["value"]
        ...
        cell_cx = ParsedParameterFile(openfoam_dir_step + "/ccx")["internalField"]
        cell_cy = ParsedParameterFile(openfoam_dir_step + "/ccy")["internalField"]
        cell_cz = ParsedParameterFile(openfoam_dir_step + "/ccz")["internalField"]
...
def test_velsP2C(prepare_config_fix, cfdprocs, mdprocs, cells,  err_msg):
    ...
    if correct:
        ...
        try:
            ...
            assert False
        compare_vels(1e-6, steps=2, dt=2, mode="test")
